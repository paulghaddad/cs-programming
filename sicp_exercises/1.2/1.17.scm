;;; Ex. 1.17

;;; The exponentiation algorithms in this section are based on performing
;;; exponentiation by means of repeated multiplication. In a similar way, one
;;; can perform integer multiplication by means of repeated addition. The
;;; following multiplication procedure (in which it is assumed that our language
;;; can only add, not multiply) is analogous to the expt procedure:

;;; This algorithm takes a number of steps that is linear in b. Now suppose we
;;; include, together with addition, operations double, which doubles an
;;; integer, and halve, which divides an (even) integer by 2. Using these,
;;; design a multiplication procedure analogous to fast-expt that uses a
;;; logarithmic number of steps.


(define (double a)
  (+ a a))

(define (halve a)
  (/ a 2))

;;; Note we are redefining the star operator
;;; Number of steps linear to b
; (define (* a b)
;   (if (= b 0)
;       0
;       (+ a (* a (- b 1)))))

;;; number of steps logarithmic to b

;;; Rules:

;;; a * b = (double a) * (halve b) if even
;;; a * b = a + (a * (b - 1)) if odd

;;; The rule for evens will halve the number of times we need to operate on b

(define (* a b)
  (cond ((or (= a 0) (= b 0)) 0)
        ((= b 1) a)
        ((= b 2) (double a))
        ((even? b) (* (double a) (halve b)))
        ((odd? b) (+ a (* a (- b 1))))))

(* 2 4) ; 8
(* 3 3) ; 9
(* 1 10) ; 10
(* 10 1) ; 10
(* 0 10) ; 0
